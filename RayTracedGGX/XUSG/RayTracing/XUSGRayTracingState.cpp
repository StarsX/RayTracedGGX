...
RayTracing::Pipeline PipelineCache::createPipeline(const string& key, const wchar_t* name)
{
	...
	auto pkeyLocalPipelineLayoutHeader = reinterpret_cast<const State::KeyLocalPipelineLayoutHeader*>
		(&key[sizeof(State::KeyHeader) + sizeof(State::KeyHitGroup) * keyHeader.NumHitGroups]);
	for (auto i = 0u; i < keyHeader.NumLocalPipelineLayouts; ++i)
	{
		...
		// Update the pointer
		pkeyLocalPipelineLayoutHeader = reinterpret_cast<const State::KeyLocalPipelineLayoutHeader*>
			(&pkeyLocalPipelineAssociation[pkeyLocalPipelineLayoutHeader->NumShaders]);
	}
